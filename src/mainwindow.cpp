...
GameRecipe selectBestRecipe(const QList<GameRecipe>& recipes)
{
    ...
    for (const auto& recipe : recipes) {
        ...
        // Проверка на корректность времени рецепта
        if (recipe.time <= 0) {
            continue; // Пропускаем некорректные рецепты
        }
      ...
      m_currentRecipeIndex(0)
{
    ...
    // Устанавливаем современный заголовок и иконку
    setWindowTitle("🏭 Satisfactory Calculator Pro | v1.0 | Полный расчет производственных цепочек");
    setWindowIcon(QIcon(":/icons/calculator.png")); // Если есть иконка
...
void MainWindow::setupUi()
{
    ...
    // Создаем информационную карточку-заголовок
    QWidget *headerCard = new QWidget();
    headerCard->setObjectName("cardWidget");
    QHBoxLayout *headerLayout = new QHBoxLayout(headerCard);
    headerLayout->setContentsMargins(24, 16, 24, 16);
    
    // Иконка и заголовок
    QLabel *titleIcon = new QLabel("🏭");
    titleIcon->setStyleSheet("font-size: 32px; margin-right: 12px;");
    
    QLabel *titleLabel = new QLabel("Satisfactory Calculator Pro");
    titleLabel->setStyleSheet(
        "font-size: 24px; font-weight: 700; color: #f1f5f9; "
        "margin: 0; letter-spacing: 0.025em;"
    );
    
    QLabel *subtitleLabel = new QLabel("Полный расчет производственных цепочек v1.0");
    ...
    QLabel *versionLabel = new QLabel("📊 Last update 1.1");
    versionLabel->setStyleSheet(
        "font-size: 12px; font-weight: 600; color: #22d3ee; "
        "background: rgba(34, 211, 238, 0.1); border-radius: 4px; "
        "padding: 4px 8px; border: 1px solid rgba(34, 211, 238, 0.2);"
    );
    
    QLabel *recipeCountLabel = new QLabel("🔧 200+ рецептов");
    recipeCountLabel->setStyleSheet(
        "font-size: 12px; font-weight: 600; color: #34d399; "
        "background: rgba(52, 211, 153, 0.1); border-radius: 4px; "
        "padding: 4px 8px; border: 1px solid rgba(52, 211, 153, 0.2);"
    );
    
    versionLayout->addWidget(versionLabel);
    versionLayout->addWidget(recipeCountLabel);
    ...
    headerLayout->addWidget(versionWidget);
    
    mainLayout->addWidget(headerCard);
    
    // Остальная часть setupUi остается без изменений
...
void MainWindow::createArrowIcons()
{
    // Создаем простые стрелочки программно
    QPixmap rightArrow(12, 12);
    rightArrow.fill(Qt::transparent);
    QPainter painter(&rightArrow);
    painter.setRenderHint(QPainter::Antialiasing);
    painter.setPen(QPen(QColor("#94a3b8"), 2));
    painter.setBrush(QBrush(QColor("#94a3b8")));
    
    // Рисуем треугольник вправо
    QPolygon triangle;
    triangle << QPoint(3, 3) << QPoint(9, 6) << QPoint(3, 9);
    painter.drawPolygon(triangle);
    
    QPixmap downArrow(12, 12);
    downArrow.fill(Qt::transparent);
    QPainter painter2(&downArrow);
    painter2.setRenderHint(QPainter::Antialiasing);
    painter2.setPen(QPen(QColor("#6366f1"), 2));
    painter2.setBrush(QBrush(QColor("#6366f1")));
    
    // Рисуем треугольник вниз
    QPolygon triangle2;
    triangle2 << QPoint(3, 3) << QPoint(9, 3) << QPoint(6, 9);
    painter2.drawPolygon(triangle2);
    
    // Сохраняем во временные файлы
    rightArrow.save("right_arrow.png");
    downArrow.save("down_arrow.png");
}
...
void MainWindow::applyStyles()
{
    ...
    // Современный дизайн в стиле веб-интерфейсов 2025
    setStyleSheet(
        "/* ===== ГЛОБАЛЬНЫЕ СТИЛИ ===== */"
        "QMainWindow {"
        "    background: qlineargradient(x1:0, y1:0, x2:1, y2:1, "
        "        stop:0 #0a0e27, stop:0.3 #1a1f3a, stop:0.7 #2a2f4a, stop:1 #1a1f3a);"
        "    color: #ffffff;"
        ...
        "        stop:0 rgba(99, 102, 241, 0.8), stop:1 rgba(79, 70, 229, 0.9));"
        ...
        "QPushButton:hover {"
        "    background: qlineargradient(x1:0, y1:0, x2:0, y2:1, "
        "        stop:0 rgba(99, 102, 241, 0.9), stop:1 rgba(79, 70, 229, 1.0));"
        "    border-color: rgba(99, 102, 241, 0.6);"
        "    transform: translateY(-1px);"
        "}"
        "QPushButton:pressed {"
        "    background: qlineargradient(x1:0, y1:0, x2:0, y2:1, "
        "        stop:0 rgba(79, 70, 229, 0.9), stop:1 rgba(67, 56, 202, 1.0));"
        ...
        "/* ===== ВЫПАДАЮЩИЕ СПИСКИ - СОВРЕМЕННЫЙ СТИЛЬ ===== */"
        "QComboBox {"
        "    background: rgba(30, 41, 59, 0.8);"
        "    border: 2px solid rgba(71, 85, 105, 0.3);"
        "    border-radius: 10px;"
        ...
        "QTabBar::tab:selected {"
        "    background: qlineargradient(x1:0, y1:0, x2:0, y2:1, "
        "        stop:0 rgba(99, 102, 241, 0.2), stop:1 rgba(79, 70, 229, 0.1));"
        "    border-color: rgba(99, 102, 241, 0.4);"
        "    color: #ffffff;"
        "}"
        "QTabBar::tab:hover:!selected {"
        ...
        "    color: #f1f5f9;"
        "    font-size: 13px;"
        "    gridline-color: rgba(71, 85, 105, 0.2);"
        "    outline: none;"
        ...
        "    background: qlineargradient(x1:0, y1:0, x2:1, y2:1, "
        "        stop:0 rgba(99, 102, 241, 1.0), stop:1 rgba(79, 70, 229, 1.0));"
        "}"
        ...
        "        stop:0 rgba(34, 197, 94, 0.8), stop:1 rgba(16, 185, 129, 0.9));"
        "    border-radius: 6px;"
        "    margin: 2px;"
        "}"
        ...
        "QWidget[objectName=\"cardWidget\"] {"
        "    background: rgba(15, 23, 42, 0.8);"
        "    border: 1px solid rgba(71, 85, 105, 0.2);"
        "    border-radius: 16px;"
        "    padding: 20px;"
        "}"
        "QWidget[objectName=\"cardWidget\"]:hover {"
    ...
    // Устанавливаем размер окна для лучшего отображения нового дизайна
    resize(1600, 1000);
    ...
    for (auto it = alternatesByItem.constBegin(); it != alternatesByItem.constEnd(); ++it) {
        ...
        for (const auto& recipe : alternates) {
            ...
            for (const auto& product : recipe.products) {
              ...
              .arg(recipe.building)
              .arg(QString::number(recipe.time, 'f', 1))
              .arg(ingredientsList)
              .arg(productsList);
...
void MainWindow::calculateLift()
{
    ...
    for (const auto &req : requirements) {
        // Используем ту же оптимальную формулу, что и ниже
        double optimalTime = sqrt(static_cast<double>(req.amount));
        optimalTime = qBound(10.0, optimalTime, 120.0);
        double productionRate = req.amount / optimalTime;
    ...
    for (const auto& req : requirements) {
        ...
        // МАТЕМАТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Оптимальная скорость = количество / sqrt(количество)
        double optimalTime = sqrt(static_cast<double>(req.amount));
        optimalTime = qBound(10.0, optimalTime, 120.0); // от 10 до 120 минут
        double productionRate = req.amount / optimalTime;
...
void MainWindow::calculateHub()
{
    ...
    for (const auto &req : requirements) {
        // Используем ту же оптимальную формулу, что и выше
        double optimalTime = sqrt(static_cast<double>(req.amount));
        optimalTime = qBound(10.0, optimalTime, 120.0);
        double productionRate = req.amount / optimalTime;
...
void MainWindow::calculateRequirements(const QString& itemClass, double amountPerMinute, QMap<QString, double>& total, int depth)
{
    ...
    if (itemsPerCraft == 0) {
        #ifdef QT_DEBUG
        qDebug() << "WARNING: Recipe produces 0 items for" << itemClass;
        #endif
        total[itemClass] += amountPerMinute;
        return;
    }
    ...
    if (recipe.time <= 0) {
        #ifdef QT_DEBUG
        qDebug() << "WARNING: Invalid recipe time for" << itemClass << ":" << recipe.time;
        #endif
        total[itemClass] += amountPerMinute;
        return;
    }
